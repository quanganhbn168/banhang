...
class SlideService
{
    ...
    public function create(Request $request): Slide
    {
        ...
        $type = (int) $data['type'];
        if ($type === Slide::TYPE_AFTER_BEFORE) {
            $data['image'] = [
                'before' => $this->uploadImage(
                    $request->file('before_image'),
                    'uploads/slides', 800, 600, true
                ),
                'after'  => $this->uploadImage(
